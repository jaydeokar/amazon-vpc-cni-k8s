...
func CalculateRouteTableId(deviceNumber int, networkCardIndex int, maxENIsPerNetworCard int) int {
	if networkCardIndex == 0 {
		return deviceNumber + 1
	} else {
		return deviceNumber + 1 + (networkCardIndex * maxENIsPerNetworCard)
	}
}
